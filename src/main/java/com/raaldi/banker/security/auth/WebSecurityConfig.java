...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
  ...
  @Autowired
  private BankerAuthenticationProvider ajaxAuthenticationProvider;
  @Autowired
  ...
  @Bean
  protected BankerSignInProcessingFilter buildAjaxLoginProcessingFilter() throws Exception {
    BankerSignInProcessingFilter filter = new BankerSignInProcessingFilter(FORM_BASED_LOGIN_ENTRY_POINT, successHandler,
  ...
  protected void configure(HttpSecurity http) throws Exception {
    http.csrf().disable() // We don't need CSRF for JWT based authentication
        ...
        .antMatchers(TOKEN_REFRESH_ENTRY_POINT).permitAll() // Token refresh
                                                            // end-point
        .antMatchers("/console").permitAll() // H2 Console Dash-board - only for
                                             // testing
