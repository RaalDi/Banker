...
public class CashRegisterRestController {
	...
	public ResponseEntity<Void> createCashRegister(@RequestBody CashRegister cashRegister, UriComponentsBuilder uriBuilder) {
		System.out.println("Creating CashRegister " + cashRegister.toString());

		if (service.exists(cashRegister)) {
			System.out.println("A CashRegister with name " + cashRegister.toString() + " already exist");
		...
		headers.setLocation(uriBuilder.path("/cashregister/{id}").buildAndExpand(cashRegister.getId()).toUri());
		return new ResponseEntity<Void>(headers, HttpStatus.CREATED);
	}

	@RequestMapping(value = "/cashregister/{id}", method = RequestMethod.PUT)
	public ResponseEntity<CashRegister> updateCashRegister(@PathVariable("id") long id, @RequestBody CashRegister cashRegister) {
